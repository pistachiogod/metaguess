...
SETUP (one time only):
  ...
  First run downloads the GloVe model (~400MB) and caches it automatically.
...
You'll be prompted to enter:
  ...
  - The date             (e.g. 2025-06-01 — or press Enter for today)
  - 3 games (title + igdb_id)

Output is appended to src/linked-database.json.
"""
...
def prompt_date() -> str:
    ...
    raw = input(f"Date (YYYY-MM-DD) [Enter for today: {today}]: ").strip()
    if not raw:
        return today
    try:
...
def main():
    ...
    existing_dates = [p["date"] for p in database]
    if puzzle_date in existing_dates:
        idx = existing_dates.index(puzzle_date)
        database[idx] = puzzle
        print(f"  ↺ Updated existing entry for {puzzle_date}")
    else:
        database.append(puzzle)
        database.sort(key=lambda p: p["date"])
        print(f"  ✓ Added puzzle for {puzzle_date}")
